...
class HistoryTestCaseConstant {
    static final String ADD_HISTORY_TEST_CASE =
            "INSERT INTO history_test_case " +
                    " (result, date_completed, id_test_case) " +
                    "  VALUES (?::test_case_result, ?, ?);";

    static final String HISTORY_TEST_CASE_FAILED_COMPLETED_PAGINATION =
            "SELECT h.id AS h_id, h.result AS h_result, h.date_completed AS " +
                    ...
                    " ORDER BY h.date_completed DESC " +
                    " LIMIT ? OFFSET ?; ";

    static final String COUNT_TEST_CASE_FAILED_COMPLETED =
            "SELECT count(*) " +
            "   FROM history_test_case " +
            "   WHERE id_test_case=? AND (result::text LIKE 'FAILED' OR result::text LIKE 'COMPLETE'); ";

    static final String EDIT_HISTORY_TEST_CASE_RESULT =
            "UPDATE history_test_case " +
                    " SET result = ?::test_case_result " +
                    " WHERE id = ?; ";
    static final String GET_BY_ID = "SELECT id, result, date_completed, id_test_case FROM history_test_case WHERE id = ?";
}
